...
# cases where triv's tr_ac_name is different from ac08_name
tr_ac_name_diff_from_08 <- triv %>%
  filter(!is.na(ac08_id) & tr_ac_name != ac08_name)

tr_ac_name_diff_from_07 <- triv %>%
  filter(!is.na(ac07_id) & tr_ac_name != ac07_name)
  # don't need to drop right now, we can use tr_ac_name because its probably the one thats linked to the election better.
  # update:
...
const_match_simple <- function() {
  ...
  common_state_year_constituencies <- inner_join(
    const_df1 %>% distinct(State, Year),
    const_df2 %>% distinct(State, Year),
    by = c("State", "Year")
  )
  
  # for each common state, match const
  for (i in 1:nrow(common_state_year_constituencies)) {
    state <- common_state_year_constituencies$State[i]
    year <- common_state_year_constituencies$Year[i]
    
    # filter both dataframes by state
    df1_const <- const_df1 %>% 
...
const_df1 <- all_states_elections_filtered %>%
  distinct(State, Year, Constituency)
...
write_csv(const_matches, "Raw Data/all_elections_triv_const_match.csv")
const_matches <- read_csv("Raw Data/all_elections_triv_const_match.csv")
...
duplicates_best <- best_const_matches %>%
  group_by(State, Year, Constituency_Original) %>%
  filter(n() > 1) %>%
  arrange(State, Year, Constituency_Original)

# drop these
best_const_matches <- best_const_matches %>%
  anti_join(duplicates_best, 
            by = c("Constituency_Original", "State", "Year"))

duplicates_best_target <- best_const_matches %>%
  group_by(State, Year, Constituency_Target) %>%
  filter(n() > 1) %>%
  arrange(State, Year, Constituency_Target)

# to be safe, we'll drop this as well
best_const_matches <- best_const_matches %>%
  anti_join(duplicates_best, 
            by = c("Constituency_Target", "State", "Year"))

triv_acid <- triv %>%
  distinct(State, Year, Constituency, ac_id)

all_states_elections_unique_const <- const_df1 %>%
  left_join(best_const_matches, 
            by = c("Constituency" = "Constituency_Original", "State", "Year"))

# drop these
sum(is.na(all_states_elections_unique_const$dist)) 
...
all_states_elections_acid <- all_states_elections_unique_const %>%
  left_join(triv_acid, 
            by = c("Constituency_Target" = "Constituency", "State", "Year"))

sum(is.na(all_states_elections_acid$ac_id))

# drop these
all_states_elections_acid <- all_states_elections_acid %>%
  filter(!is.na(ac_id))

duplicates_all_states_acid <- all_states_elections_acid %>%
  group_by(State, Year, Constituency_Target, ac_id) %>%
  filter(n() > 1) %>%
  arrange(State, Year, Constituency_Target)

# drop these
all_states_elections_acid <- all_states_elections_acid %>%
  anti_join(duplicates_all_states_acid, 
            by = c("Constituency_Target" = "Constituency", "State", "Year"))

unique_const_state <- all_states_elections_filtered %>%
  distinct(State, State_Name, Constituency, Constituency_Name, Year, month)

all_states_elections_acid_final <- all_states_elections_acid %>%
  left_join(unique_const_state %>%
              dplyr::select(State, State_Name, Constituency, Constituency_Name, Year, month), 
            by = c("State", "Constituency", "Year"))

duplicates_all_states_acid_final <- all_states_elections_acid_final %>%
  group_by(State, Constituency, Year) %>%
  filter(n() > 1) %>%
  arrange(State, Constituency, Year)
  # because of by elections

# drop these
all_states_elections_acid_final <- all_states_elections_acid_final %>%
  anti_join(duplicates_all_states_acid_final, 
            by = c("State", "Constituency", "Year"))
...
candidate_ballot_order <- all_states_elections_filtered %>%
...
constituency_election_metrics <- candidate_pairs %>%
  group_by(State, Year, Constituency_Name, Election_Type, Assembly_No) %>%
  dplyr::summarize(
...
constituency_election_metrics <- constituency_election_metrics %>%
  left_join(all_states_elections_acid_final %>%
              dplyr::select(State, Constituency_Name, Year, photo), 
            by = c("State" = "State", 
                   "Constituency_Name" = "Constituency_Name", 
                   "Year" = "Year"))
...
constituency_election_metrics <- constituency_election_metrics %>%
  left_join(all_states_elections_acid_final %>%
              dplyr::select(State, Constituency_Name, Year, ac_id), 
            by = c("State" = "State", 
                   "Constituency_Name" = "Constituency_Name", 
                   "Year" = "Year"))
