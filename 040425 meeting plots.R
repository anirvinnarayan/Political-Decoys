# ============================================
# Political Decoys - 040425 meeting plots
# ============================================
# Date: 04/04/25
...
lv_975th <- quantile(candidate_pairs$Levenshtein_Similarity, 0.975, na.rm = TRUE)
jw_975th <- quantile(candidate_pairs$Jaro_Winkler_Similarity, 0.975, na.rm = TRUE)
mp_975th <- quantile(candidate_pairs$Metaphone_Similarity, 0.975, na.rm = TRUE)
mas_975th <- quantile(candidate_pairs$Masala_Similarity, 0.975, na.rm = TRUE)
ng_975th <- quantile(candidate_pairs$NGram_Similarity, 0.975, na.rm = TRUE)
...
candidate_pairs_alternative_1 <- candidate_pairs %>%
  mutate(is_decoy = ifelse(
    # all measures must be above 97,5th percentile thresholds
    Levenshtein_Similarity > lv_975th & 
      Jaro_Winkler_Similarity > jw_975th & 
      Metaphone_Similarity > mp_975th & 
      Masala_Similarity > mas_975th & 
      NGram_Similarity > ng_975th &
      # AND at least one measure must be above 99th percentile threshold
      (Levenshtein_Similarity > lv_99th | 
         Jaro_Winkler_Similarity > jw_99th | 
         Metaphone_Similarity > mp_99th | 
         Masala_Similarity > mas_99th | 
...
candidate_pairs_alternative_2 <- candidate_pairs %>%
  mutate(is_decoy = ifelse(
    # all measures must be above 97,5th percentile thresholds
    Levenshtein_Similarity > lv_975th & 
      Jaro_Winkler_Similarity > jw_975th & 
...
### Alternative 1
### how are main_main, main_minor and minor_minor different? false positives? 
str(candidate_pairs_alternative_1)
candidate_pairs_alternative_1_main_minor <- candidate_pairs_alternative_1 %>%
  filter(Pair_Type == "main-minor" | Pair_Type == "minor-main", 
         is_decoy == TRUE)

candidate_pairs_alternative_1_main_main <- candidate_pairs_alternative_1 %>%
  filter(Pair_Type == "main-main", 
         is_decoy == TRUE)

candidate_pairs_alternative_1_minor_minor <- candidate_pairs_alternative_1 %>%
  filter(Pair_Type == "minor-minor", 
         is_decoy == TRUE)

minor_candidates_that_are_decoys_of_minor <- unique(c(
  candidate_pairs_alternative_1_minor_minor$Candidate1_PID,
  candidate_pairs_alternative_1_minor_minor$Candidate2_PID
))

minor_candidates_that_are_decoys_of_main <- unique(c(
  candidate_pairs_alternative_1_main_minor %>% 
    filter(Pair_Type == "minor-main") %>% 
    pull(Candidate1_PID),
  candidate_pairs_alternative_1_main_minor %>% 
    filter(Pair_Type == "main-minor") %>% 
...
elections_with_main_minor_decoys <- candidate_pairs_alternative_1 %>%
  filter((Pair_Type == "main-minor" | Pair_Type == "minor-main") & is_decoy == TRUE) %>%
  distinct(Year, State_Name, Constituency_Name, Assembly_No, Election_Type) %>%
  nrow()

elections_with_main_main_decoys <- candidate_pairs_alternative_1 %>%
  filter(Pair_Type == "main-main" & is_decoy == TRUE) %>%
  distinct(Year, State_Name, Constituency_Name, Assembly_No, Election_Type) %>%
  nrow()

# removed overlap minor candidates! 
elections_with_minor_minor_decoys <- candidate_pairs_alternative_1 %>%
  filter((Pair_Type == "minor-minor") & is_decoy == TRUE) %>%
  filter(!(Candidate1_PID %in% minor_candidates_that_are_decoys_of_main | 
             Candidate2_PID %in% minor_candidates_that_are_decoys_of_main)) %>%
  distinct(Year, State_Name, Constituency_Name, Assembly_No, Election_Type) %>%
...
total_elections <- candidate_pairs_alternative_1 %>%
  distinct(Year, State_Name, Constituency_Name, Assembly_No, Election_Type) %>%
  nrow()
...
decoy_by_pair_type <- candidate_pairs_alternative_1 %>%
  # creae a new consolidated pair type variable
  mutate(
    Consolidated_Pair_Type = case_when(
      Pair_Type == "main-minor" ~ "main-minor/minor-main",
...
### Alternative 2
### how are main_main, main_minor and minor_minor different? false positives? 
str(candidate_pairs_alternative_2)
candidate_pairs_alternative_2_main_minor <- candidate_pairs_alternative_2 %>%
  filter(Pair_Type == "main-minor" | Pair_Type == "minor-main", 
         is_decoy == TRUE)

candidate_pairs_alternative_2_main_main <- candidate_pairs_alternative_2 %>%
  filter(Pair_Type == "main-main", 
         is_decoy == TRUE)

candidate_pairs_alternative_2_minor_minor <- candidate_pairs_alternative_2 %>%
  filter(Pair_Type == "minor-minor", 
         is_decoy == TRUE)

minor_candidates_that_are_decoys_of_minor <- unique(c(
  candidate_pairs_alternative_2_minor_minor$Candidate1_PID,
  candidate_pairs_alternative_2_minor_minor$Candidate2_PID
))

minor_candidates_that_are_decoys_of_main <- unique(c(
  candidate_pairs_alternative_2_main_minor %>% 
    filter(Pair_Type == "minor-main") %>% 
    pull(Candidate1_PID),
  candidate_pairs_alternative_2_main_minor %>% 
    filter(Pair_Type == "main-minor") %>% 
...
elections_with_main_minor_decoys <- candidate_pairs_alternative_2 %>%
  filter((Pair_Type == "main-minor" | Pair_Type == "minor-main") & is_decoy == TRUE) %>%
  distinct(Year, State_Name, Constituency_Name, Assembly_No, Election_Type) %>%
  nrow()

elections_with_main_main_decoys <- candidate_pairs_alternative_2 %>%
  filter(Pair_Type == "main-main" & is_decoy == TRUE) %>%
  distinct(Year, State_Name, Constituency_Name, Assembly_No, Election_Type) %>%
  nrow()

# removed overlap minor candidates! 
elections_with_minor_minor_decoys <- candidate_pairs_alternative_2 %>%
  filter((Pair_Type == "minor-minor") & is_decoy == TRUE) %>%
  filter(!(Candidate1_PID %in% minor_candidates_that_are_decoys_of_main | 
             Candidate2_PID %in% minor_candidates_that_are_decoys_of_main)) %>%
  distinct(Year, State_Name, Constituency_Name, Assembly_No, Election_Type) %>%
...
total_elections <- candidate_pairs_alternative_2 %>%
  distinct(Year, State_Name, Constituency_Name, Assembly_No, Election_Type) %>%
  nrow()
...
decoy_by_pair_type <- candidate_pairs_alternative_2 %>%
  # creae a new consolidated pair type variable
  mutate(
    Consolidated_Pair_Type = case_when(
      Pair_Type == "main-minor" ~ "main-minor/minor-main",
